package com.taz.accessability.meditrack.constants;

/**
 * Created by tahzuddin on 05/06/17.
 */

public class Constants {
    public static String   EDIT_USER_INFO = "edit_user_info";
    public static String   EDIT_MEDI_INFO = "edit_medi_info";
    public static String   ADD_MEDI_INFO = "add_medi_info";


    public static int  START_ACTIVITY_FOR_RESULT_EDIT_USER_INFO = 1;
    public static int  START_ACTIVITY_FOR_RESULT_EDIT_MEDIA_INFO = 2;
    public static int  START_ACTIVITY_FOR_RESULT_ADD_MEDIA_INFO = 3;


    public static String   SETTINGS = "Settings";
    public static String   ALL_MEDICINE = "All Medicines";
    public static String   TODAYS_MEDICINES = "Today's Medicines";
    public static String   EDIT_INFO = "Edit Info";
    public static String   EDIT_MEDICINE = "Edit Medicine";
    public static String   ADD_MEDICINE = "Add Medicine";
    public static String   MEDICINE = "medicine";
}
